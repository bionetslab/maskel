...
**One pipeline, deliberately napari-free.** [maskel/pipeline.py](maskel/pipeline.py)'s `analyze_binary_image()` is the single place the full per-image algorithm runs. It returns a plain-data `AnalysisResult` (skeleton, per-object graphs, branch/node records, summary features, radius matrix) with no napari dependency anywhere, not even under `TYPE_CHECKING`. The CLI's [maskel/_batch.py](maskel/_batch.py) (used across a `ProcessPoolExecutor`) calls it directly; napari-maskel calls the same function and builds its own `LayerDataTuple`s on top of the result in its own package. Never reintroduce a napari import here — that coupling was deliberately removed when the napari plugin was split into its own repo.

**Per-object splitting happens before the per-object pipeline runs.** `analyze_binary_image()` first calls `_iter_object_crops()`, which treats a plain binary mask as one implicit object (id `1`) or, when the input has more than one distinct nonzero value, treats each value as its own object (an instance segmentation map). Each object is cropped to its own bounding box (padded by 1px, clamped to the array) and run through `_analyze_single_object()` — the actual per-object pipeline, described below — independently. This is why touching-but-distinct labeled objects end up correctly skeletonized as separate structures rather than merged into one: `mask == object_id` never includes a neighboring object's pixels, regardless of adjacency. Results are stitched back: `skeleton`/`radius_matrix`/`preprocessed_binary` are pasted into full-image-sized arrays (via `np.maximum`, not plain assignment — two objects' 1px-padded crops can overlap at a shared border), and every branch/node record and summary-features dict is tagged with `object_id` (the mask's own label value) with node coordinates shifted by the crop's offset back into global image coordinates. `summary_features` is a list with one dict per object, not a single whole-image dict — computing per-object stats (like `fractal_dimension`/`vessel_area_fraction`) on each object's own crop is what makes this correct, since those are scale-dependent and would mean something different computed globally. `AnalysisResult.object_graphs` holds each object's own `skan.Skeleton`/branch-data pair plus its crop offset (`ObjectGraph`) — needed by napari-maskel (to place branch/node layers in global coordinates without a single unified graph) and by graphml export (one file per object; skan/networkx have no notion of a multi-object disjoint graph).
